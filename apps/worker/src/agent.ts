...
export class AgentExecutionError extends Error {
  constructor(
    message: string,
    readonly contextTraceJson: string,
  ) {
    super(message);
...
export async function executeAgentTool(input: {
  ...
  try {
    if (
      !isAllowed({ allowedTools: input.allowedTools, toolName: input.toolName })
    ) {
      result = {
      ...
      if (dueTimeIso) {
        ...
        if (Number.isFinite(parsed)) {
          ...
          throw new Error(
            "Invalid dueTimeIso format. Please use ISO 8601 string.",
          );
        }
      }
      ...
      let responseText = todo
        ? `已创建待办 #${todo.id}：${todo.title}`
        : "待办内容不能为空。";
      if (todo && dueAt) {
      ...
      result = {
        ...
        output: todo
          ? { id: todo.id, title: todo.title, dueAt: todo.dueAt }
          : { created: false },
      };
        ...
        responseText:
          todos.length === 0
            ...
            : [
                ...
                ...todos.map(
                  (todo) =>
                    `#${todo.id} ${todo.title}${todo.dueAt ? ` (到期时间: ${new Date(todo.dueAt).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" })})` : ""}`,
                ),
              ].join("\n"),
        toolName: "list_todos",
        riskLevel: "read",
      ...
      const content = stringArg(input.args, "content") ?? "";
      let userProfile = await input.runtime.repositories.getUserProfile(
      ...
      const results = await input.runtime.searchClient.search({
        query,
        count: 5,
      });
      result = {
        ...
        if (input.runtime.llmClient) {
          try {
            const summaryResponse =
              await input.runtime.llmClient.createChatCompletion({
                messages: [
                  {
                    role: "system",
                    content:
                      "You are a helpful assistant. Please summarize the following long web page content. You must retain the core information, main arguments, and specific facts so they can be cited later. If the content seems to be truncated halfway, summarize what is available.",
                  },
      ...
      const scenario =
        (stringArg(input.args, "scenario") as PersonalModelScenario | "") ||
        "global";
      ...
      const content = stringArg(input.args, "content");
      const metadata =
        (input.args.metadata as Record<string, unknown> | undefined) || {};
      const resolveGapId = input.args.resolveGapId
        ? String(input.args.resolveGapId)
        : null;
      const claims =
        (input.args.claims as
          | Array<{
              claim: string;
              layer: string;
              scenario: string;
      ...
      if (sourceType && !allowedAgentTypes.includes(sourceType)) {
        throw new Error(
          `Agent is only allowed to save source types: ${allowedAgentTypes.join(", ")}`,
        );
      }
      ...
      if (sourceType && title && content) {
        ...
        source =
          await input.runtime.repositories.createPersonalModelSourceDocument({
            id: generateId(),
            ownerTgUserId: input.ownerTgUserId,
...
const toolDefinitions: Record<BuiltInToolName, LlmToolDefinition> = {
      ...
      parameters: {
        ...
        properties: {
          ...
          dueTimeIso: {
            type: "string",
            description:
              "ISO 8601 format string representing the due time (e.g. 2026-06-03T15:00:00+08:00). Must include timezone offset. Can be omitted if no due time is specified.",
          },
        },
        required: ["title"],
      },
  ...
  update_core_memory: {
    ...
    function: {
      ...
      description:
        "Update the core memory markdown document of the user. Overwrites the existing core memory.",
      parameters: {
        type: "object",
        properties: {
          content: {
            type: "string",
            description: "The full markdown content for the core memory.",
          },
        },
        required: ["content"],
      },
  ...
  web_search: {
    ...
    function: {
      name: "web_search",
      description:
        "Search the public web. Cite result URLs in the final answer.",
      parameters: {
        type: "object",
  ...
  fetch_url: {
    ...
    function: {
      name: "fetch_url",
      description:
        "Fetch text from an http or https URL. Cite the URL if used.",
      parameters: {
        type: "object",
  ...
  record_understanding_gap: {
    ...
    function: {
      ...
      description:
        "Record a gap in your understanding about the user when you realize you need more information to assist them properly.",
      parameters: {
        type: "object",
        properties: {
          scenario: {
            type: "string",
            description:
              "The scenario, e.g. global, writing, relationship, etc.",
          },
          gapDescription: {
            type: "string",
            description: "What you don't know and need to find out.",
          },
        },
  ...
  record_metacognition_log: {
    ...
    function: {
      ...
      description:
        "Record a reflection or correction when the user points out a misunderstanding or corrects your previous assumptions.",
      parameters: {
        type: "object",
        properties: {
          content: {
            type: "string",
            description:
              "Detailed description of what you misunderstood and what the correct understanding is.",
          },
        },
        required: ["content"],
      },
  ...
  save_interview_source: {
    ...
    function: {
      ...
      description:
        "Save structured interview results or dynamic conversation findings as a source document and automatically resolve the corresponding gap.",
      parameters: {
        type: "object",
        properties: {
          sourceType: {
          ...
          title: {
            type: "string",
            description:
              "A short descriptive title for this record, e.g., 'Personality Framework MBTI Interview'.",
          },
          content: {
            type: "string",
            description:
              "The structured content or summarized text to save. Use markdown headers or sections for clarity.",
          },
          metadata: {
            type: "object",
            description:
              "Optional metadata matching the sourceType schema (e.g. frameworkType/agreementLevel for personality_framework).",
          },
          resolveGapId: {
            type: "string",
            description:
              "Optional Gap ID to automatically resolve once this source is successfully saved.",
          },
          claims: {
            type: "array",
            items: {
              ...
              properties: {
                claim: {
                  type: "string",
                  description: "The high-confidence conclusion or fact.",
                },
                layer: {
                  type: "string",
                  enum: ["core", "preference", "behavior", "context"],
                  description: "The layer of the claim.",
                },
                scenario: {
                  type: "string",
                  description:
                    "The applicable scenario (e.g., global, health, relationship).",
                },
                confidence: {
                  type: "string",
                  enum: ["low", "medium", "high"],
                  description:
                    "Confidence score (low, medium, or high). For confirmed facts, use high.",
                },
              },
              required: ["claim", "layer", "scenario", "confidence"],
            },
            description:
              "Optional list of high-confidence claims to directly inject into the personal model alongside the raw source document.",
          },
        },
  ...
  submit_answer: {
    ...
    function: {
      name: "submit_answer",
      description:
        "Submit the final answer. You MUST use this tool to provide the final answer if you have used web_search or fetch_url. You MUST provide citations for the facts included in your answer.",
      parameters: {
        type: "object",
        properties: {
          text: {
            type: "string",
            description: "The final answer text in Markdown format.",
          },
          citations: {
            type: "array",
            items: {
              ...
              properties: {
                ...
                snippet_used: {
                  type: "string",
                  description:
                    "The exact snippet or specific fact used from the source.",
                },
              },
              required: ["url", "title", "snippet_used"],
            },
            description:
              "List of citations used to support the facts in the answer.",
          },
        },
...
function shouldRequestExecutionPlan(input: {
  ...
  return toolHints.some(
    ([toolName, pattern]) =>
      input.allowedToolNames.includes(toolName) && pattern.test(text),
  );
}

function isControlledToolName(
  toolName: string,
): toolName is ControlledToolName {
...
export async function executeLlmAgent(
  ...
  for (const claimId of contextAssembly.trace.selectedClaimIds) {
    await input.runtime.repositories.createPersonalModelEvent({
      ...
      payloadJson: JSON.stringify({
        runId: input.runId,
        scenario: contextAssembly.trace.scenario,
      }),
      createdAt: input.runtime.now(),
    });
  }
  ...
  const candidateControlledToolNames =
  ...
  const shouldPlan = routeDecision
    ? routeDecision.mode === "plan_guided" &&
      candidateControlledToolNames.length > 0
  ...
  function recordActualToolCall(round: number, toolName: string): boolean {
    ...
    contextAssembly.trace.planDeviations.push({
      ...
      reason: expected
        ...
        : "tool_call_after_plan_exhausted",
    });
    return false;
  }
  ...
  function executionError(message: string): AgentExecutionError {
    return new AgentExecutionError(
      message,
      JSON.stringify(contextAssembly.trace),
    );
  }
          ...
          if (
            aliases.some(
              (alias) =>
                ...
                parsed.hostname.endsWith(`.${alias}`),
            )
          ) {
            return true;
          }
  ...
  if (plan.status === "generated" && plan.steps.length > 0) {
    ...
    planInstruction =
      "Planner 判断当前请求不需要调用工具，请直接回答或询问澄清问题。";
  }
  ...
  const systemInstructions = [
    ...
    hasInitialGaps
      ? "【强主动拦截规则】系统存在未解决的核心初始化Gap，你当前的最高优先级是发起或继续Onboarding访谈，强引导用户回答以填补这些Gap，而不是单纯回应闲聊。"
      : "",
  ...
  const recentRuns = await input.runtime.repositories.listRunsForSession(
    input.ownerTgUserId,
    input.sessionId,
  );
  const activeSession = await input.runtime.repositories.getActiveChatSession(
    input.ownerTgUserId,
  );

  let themeSummary = activeSession?.themeSummary || null;
  let unsummarizedRuns = recentRuns.filter(
    (r) =>
      r.status === "succeeded" &&
      r.messageText &&
      r.responseText &&
  ...
  if (activeSession?.summarizedUpToRunId) {
    const summarizedIndex = unsummarizedRuns.findIndex(
      (r) => r.id === activeSession.summarizedUpToRunId,
    );
  ...
  if (unsummarizedRuns.length > MAX_UNSUMMARIZED_RUNS) {
    ...
    const oldSummaryText = themeSummary
      ? `【当前主题摘要】\n${themeSummary}\n`
      : "";
    const newInteractionsText = runsToCompress
...
${newInteractionsText}`;
  ...
  for (let round = 0; round <= input.maxToolRounds; round += 1) {
    ...
    if (completion.toolCalls.length === 0) {
      if (
        visitedUrls.size > 0 &&
        !completion.content.includes(UNTRUSTED_INSTRUCTION_WARNING)
      ) {
    ...
    for (const toolCall of completion.toolCalls) {
      await input
        .onThinking?.({ type: "tool", toolName: toolCall.function.name })
        .catch(() => {});
      const toolCallAllowedByPlan = recordActualToolCall(
        round,
      ...
      if (toolCall.function.name === "submit_answer") {
        ...
        const citations = Array.isArray(toolArgs.citations)
          ? toolArgs.citations
          : [];
        ...
        if (hasUsedSearch) {
          ...
          for (const cite of citations) {
            ...
            const citeUrl =
              typeof citeRaw === "string"
                ? normalizeExternalUrl(citeRaw)
                : null;

            let isValid = false;
            if (citeUrl) {
              const explicitUrls =
                input.plannerRouteDecision?.fetchPolicy.explicitAllowedUrls ||
                [];
              if (
                visitedUrls.has(citeUrl) ||
                explicitUrls.some(
                  (u: string) => normalizeExternalUrl(u) === citeUrl,
                )
              ) {
                isValid = true;
              } else {
                try {
                  const citeDomain = new URL(citeUrl).hostname.replace(
                    /^www\./,
                    "",
                  );
                  ...
                  for (const domain of validDomains) {
                    if (
                      domain === citeDomain ||
                      domain.endsWith(`.${citeDomain}`) ||
                      citeDomain.endsWith(`.${domain}`)
                    ) {
                      isValid = true;
                      break;
                    }
                    if (
                      (domain.includes("githubusercontent.com") &&
                        citeDomain.includes("github.com")) ||
                      (domain.includes("github.com") &&
                        citeDomain.includes("githubusercontent.com"))
                    ) {
                      isValid = true;
                      break;
                    }
      ...
      try {
        if (guardrail.action !== "allow") {
            ...
            const results =
              (
                toolResult.output as {
                  results?: Array<{ title: string; url: string; rank: number }>;
                }
              ).results ?? [];
          ...
          if (toolCall.function.name === "fetch_url") {
            ...
            const normalizedUrl = fetched.url
              ? normalizeExternalUrl(fetched.url)
              : null;
            if (normalizedUrl) {
              visitedUrls.add(normalizedUrl);
            }
