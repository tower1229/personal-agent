...
| T7 | Telegram 长任务进度消息 | 完成 | P1 | 与 T2 相关，但可作为独立 UX 专项推进。已确认复用底层机制并通过单消息更新+Inline Keyboard防刷屏。 |
...
- **已闭环**。长任务由于时间跨度大，已通过单消息状态刷新和行内按钮的方式完成了完整的 Telegram 端体验。
...
- 设计统一的进度通知策略：
  ...
  - 失败和可恢复建议。
  - 最终完成。
- **避免频繁刷屏，支持合并或节流**：通过在 Telegram 采用 `editMessageText` 对同一条消息原地更新进度，完美避免了分步推送导致的刷屏问题。
- **可恢复与交互响应**：利用 Inline Keyboard 实现了任务 `waiting_for_user` 时期的 `[✅ 确认]` / `[⏭️ 跳过]` / `[❌ 取消]`，以及报错时的 `[🔄 重试]`，并且能够联动底层 `Paused` 等任务状态继续执行。

### 验收口径

- 一个多步后台任务至少能收到计划、关键进度、完成三类消息。（已通过单条进度消息刷新满足）
- 短任务不发送多余进度。
- 失败任务会说明失败步骤和下一步选择。（已通过错误反馈+重试/取消按钮满足）
