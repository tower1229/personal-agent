...
2. `handleUserTextMessage` 创建 `runs` 记录，状态为 `running`。
3. 如果消息是 `确认 <code>` 或 `取消`，先进入 approval 决策分支。
4. 如果消息触发 `生成今日简报`、`今日简报` 或 `daily brief`，进入 workflow 分支。
5. 其他消息进入 Agent runtime，LLM 可选择调用 tools。
6. Tool registry 校验参数、判断风险、执行工具或创建 approval request。
7. messageHandler 将 run 标记为 `succeeded` 或 `failed`，记录输出、错误和耗时。
8. Bot 用最终回复覆盖 progress message。
...
2. 校验扩展名和 2MB 文件大小限制。
3. 下载文件并按 UTF-8 文本解析。
4. 调用 `ingestDocument` 写入 `documents`、`document_chunks`，并尽力写入 chunk embeddings。
5. 对重复内容按 hash 跳过导入。
6. 创建对应 run，记录成功或失败状态。
...
- `destructive` 操作要求确认码，用户必须回复 `确认 <code>`。
- `取消` 会拒绝 pending approval；过期 approval 会被标记为 `expired`。
- Admin Dashboard 只读，不执行 approval。

### RAG

- 文档入库保存原文 hash、chunks、metadata 和可选 embeddings。
- 检索优先使用 keyword + vector hybrid score；embedding 不可用时 fallback 到 keyword。
