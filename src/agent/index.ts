  ...
  const messages: LlmMessage[] = [
    {
      ...
      content: [
        ...
        "After save_memory returns status=created, include the word 记住 in the final reply.",
        "If save_memory returns status=duplicate, do not say a new memory was created; say this is already remembered.",
        "If save_memory returns status=merged or status=updated, say the existing memory was updated.",
        "If a memory tool reports a conflict, ask the user whether to replace the older memory before changing it.",
        ...
        "When answering from documents, use search_documents first and prioritize the returned chunks over memory or general knowledge.",
        "Use only the returned chunks as document evidence. Do not invent sources, document titles, chunk indexes, or claims not supported by retrieved chunks.",
        "If search_documents returns no chunks, clearly say you did not find relevant information in saved documents.",
        "If the returned chunks are weak, indirect, or insufficient for the exact question, clearly say there is not enough evidence in saved documents.",
        "After a document-grounded answer, add a short evidence line in this format: 依据：<sourceTitle> / chunk <chunkIndex>.",
        "If multiple chunks support the answer, cite only the most relevant titles and chunk indexes briefly.",
